...
namespace eval GUI {
	...
	proc newGUI { base baseVisor baseConf } {
		...
		$menu add cascade -label "Tinker" -menu $menu.tinker
			...
			$mentink add cascade -label "Busqueda Conformacional" -menu $mentink.busqconform 
				...
				$menubc add command -label "Busqueda Conformacional (Newton)" -command "GUI::modelizarTinker $base $baseConf 1" -command "GUI::modelizarTinker $base $baseConf 1" 
				$menubc add command -label "Busqueda Conformacional (Optimizar)" -command "GUI::modelizarTinker $base $baseConf 2" -command "GUI::modelizarTinker $base $baseConf 2" 
	...
	proc ventanaCteAcoplamiento { base } {
		...
		if { $ang == -1 || [cteAcoplamientoValida $base] == -1} {
			tk_messageBox -type ok -title "Atencion" -icon warning -title "Angulo Diedro no seleccionado" -message "Seleccione 4 atomos correctos para calcular la Cte de Acoplamiento" 
		} else {
			set mainf ".cteacopl"
			...
			set label [Label $framecent.lab -activebackground #a0a0a0 -text "Angulo Diedro :   $ang \º" -font {Arial 10 {bold roman}} -relief flat -anchor w]
	...
	proc ventanaMapaReaccion { base } {
		...
		#f1
		set l3a [Label $framecent3a.lea -activebackground #a0a0a0 -text "Atomo 1 : " -font {Arial 9 {bold roman}} -relief flat -anchor center]
		...
		#f2
		set l3b [Label $framecent3b.leb -activebackground #a0a0a0 -text "Atomo 2 :  " -font {Arial 9 {bold roman}} -relief flat -anchor center]
			...
			if {$f1of2 == 1} {
				...
				if {$gui(baseDataMR1) != ""} {
					...
					if {[string match $formula1 $formula2]} {
						...
						tk_messageBox -type ok -title "Atencion" -icon warning -message "La formula molecular de \"$filename\" no coindice con la de \"$gui(ficheroMR1)\"" -parent $gui(mainfMR)
					}
				} else {
					tk_messageBox -type ok -title "Atencion" -icon warning -message "Define antes el Fichero 1" -parent $gui(mainfMR)
				}
			}
		}
	...
	proc manejadorEjecutarMR { base } {
		...
		if { $gui(baseDataMR1) != "" && $gui(baseDataMR2) != ""} {
			...
			if {[llength $gui(listaAsignadosMR1)] != $datos(numAtomos)} {
				tk_messageBox -type ok -title "Atencion" -icon warning -message "Asignacion no completada. Asigne los atomos restantes e intentelo de nuevo"
			} else {
			...
			tk_messageBox -type ok -title "Atencion" -icon warning -message "Define los ficheros y realiza la asignacion"
		}
	}; #finproc

	proc manejadorCancelarMR { base } {
		upvar #0 GUI::$base gui
		if {[tk_messageBox -type yesno -title "Atencion" -icon warning -message "Esta seguro de que desea salir y perder el trabajo"]} {
			destroy $gui(mainfMR)
		}
	}
#----------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------		
	...
	proc manejadorLBRE { base } {
		...
		if {$gui(modoInteraccion) == "rotar"} {
			...
			$menu add separator
			$menu add check -label "Proyeccion Paralela/Conica" -variable "GUI::${base}(proyeccion)" -command "GUI::manejadorProyeccion $base"
	...
	proc cargarFicheroMol { base filename } {
		...
		if { $erro == ""} {
			...
			tk_messageBox -type ok -icon warning -title "Atencion" -message "Error al leer el Fichero $filename, compruebe q el formato es correcto" 
			return "ERROR"
		}
	}; #finproc
	...
	proc guardarComo { base file } {
		...
		if {[llength $listaM] == 1} {
			...
			if {$file == 0} {
				set op [tk_messageBox -title "Informacion" -message "¿Desea agrupar todas las moleculas en el mismo fichero?" -type yesnocancel -icon question]
			} else { set op "yes"}
			if {$op == "yes"} {
				if {$file == 0} {
					set filename [tk_getSaveFile -filetypes $types -defaultextension mol -initialdir $config(dirData) -title "Guardar Fichero Mol"]
	...
	proc modelizarTinker { base baseConfig tipo } {
		...
		if {$gui(semaforoProceso) == 0} {
			...
			tk_messageBox -title "Informacion" -message "Existe un calculo en proceso, detengalo o espere a que finalize."
		}
	}; #finproc
...
#----------------------------------------------------------------------------------------------		
	...
	proc ventanaConfiguracion { base } {
		...
		set con [checkbutton $fcon.conchk -offvalue "hide" -onvalue "show" -text "Mostrar Consola al Iniciar" -font {Arial 9 {bold roman}} -variable Conf::$gui(conf)(estadoConsola)]
		pack $con -side left
	...
	proc guardarPropiedades { base } {
		...
		tk_messageBox -type ok -title "Informacion" -message "Los cambios se efectuaran la proxima vez que inicie BrandyMol"
...
#----------------------------------------------------------------------------------------------		
	...
	proc acercaDe { base } {
		...
		set texto3 "    Brandy Mol ha sido desarrollado como PFC \n \
