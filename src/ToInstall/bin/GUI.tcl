...
#								   --------------------
# 	Implementacion de la GUI y nucleo de Brandy Mol
# 
#
#	Definido dentro del
#			namespace eval GUI
...
#
#		base(consola) 			--> 0 : consola oculta, 1 : consola mostrada
#		
#		base(semaforoProceso) 	--> 0: semaforo de proceso libre
#		base(labelSemaforo) 	--> etiqueta indicadora del estado de proceso
#		
#		base(semillaLogs) 			--> semilla para la generacion de logs
#		base(semillaFichCargados) 	--> semilla para la generacion de identificadores de moleculas
#		base(fichCargados)			--> lista de identificadores de moleculas cargadas
#		
#		
...
#		
#		base(cancelarProceso) 	--> 1 : si el proceso en curso ha sido cancelado, 0 : en caso contrario
#		base(varProgressBar) 	--> valor de la ProgressBar
#		base(resultadoTink) 	--> variable q almacenara el resultado de Tinker
#		base(resultadoMopac)  	--> variable q almacenara el resultado de Mopac
#		
#		
#		base(proyeccion)  		--> 0 : perspectiva paralela, 1 : conica
#		base(resolucion) 		--> resolucion activa : B, M, A ó U
#		base(modoInteraccion)  	--> modo de interaccion activo : 
#		base(labels) 			--> 0 : etiquetas desactivadas, 1 : activadas
#		
#		set base(ejes) 			--> 0 : ejes no visibles, 1: visibles
#		
#		#para la configuracion
#		base(mainConfig) 		--> ventana de configuracion	
#		base(colores) 			--> lista de colores
#		base(elementos) 		--> lista de elementos
#		base(listBoxColors) 	--> listBox de colores
#		base(labelColor) 		--> label para la etiqueta de colores
#		base(dirData) 			--> directorio de datos usado en la configuracion
#		
#		base(colorEtq) 			--> color para las etiquetas
#		base(labelColorEtq) 	--> label para el color de las etiquetas
#		
#		base(colorFondo) 			--> color de fondo
#		base(labelColorFondo) 		--> etiqueta para el color de fondo
#		
#		base(etqId) 				--> mostrar Identificadores
#		base(etqCodB)  				--> mostrar Cod BrandyMol
#		base(etqCarga) 				--> mostrar Carga
#		base(etqCodTink) 			--> mostrar codigo tinker
#		base(etqQuira) 				--> mostrar quiralidad
#		
#		
#		#la escala del cpk
#		base(escalaCPK) 		--> entero para la escala del CPK
#		base(scaleWCPK) 		--> widget scale para la escala del CPK
#		
#		
#		#para las propiedades 
#		base(mainProp) 			--> ventana de propiedades
#		
#		#para los puentes de hidrogeno
#		base(mainPH) 		--> ventana de Puentes de hidrogeno
#		base(phO) 			--> incluir Oxigeno en PH
#		base(phN) 			--> incluir Nitrogeno en PH
#		base(phS) 			--> incluir Azufre en PH
#		base(phF) 			--> incluir Fluor en PH
#		base(phDist) 		--> distancia minima de formacion para los puentes de hidrogeno
#		
#		#para la edicion de tinker
#		base(mainfEdTink) 	--> ventana para la edicion del Tinker.key
#		
#		
#		#para la superficie
#		base(repreSuperficie)"	--> estado de representacion para las superficies
#		
#		#para el mapa de reaccion
#		base(mainfMR) 		--> ventana para el mapa de reaccion
#		base(ficheroMR1) 	--> fichero mol 1 para el MR
#		base(ficheroMR2) 	--> fichero mol 2 para el MR
#		base(baseDataMR1) 	--> Data del mol 1 del MR
#		base(baseDataMR2) 	--> Data del mol 2 del MR
#		base(listBoxMR1) 	--> ListBox del mol 1 del MR
#		base(listaBoxMR2) 	--> ListBox del mol 2 del MR
#		base(listBoxMRF) 	--> ListBox para el resultado del MR
#		
#		base(listaAsignadosMR1) 	--> lista de atomos asignados del mol 1 del MR
#		base(listaAsignadosMR2) 	--> lista de atomos asignados del mol 2 del MR
#		
#		#para el acerca de
#		base(mainAcercaDe) 	--> ventana AcercaDe
#
#
#
#	Define :
...
namespace eval GUI {
	...
	proc newGUI { base baseVisor baseConf } {
		...
		set gui(proyeccion) 0
		set gui(resolucion) "M"
		...
		$menu add cascade -label "Archivo" -menu $menu.archivo
			...
			$menarch add separator
			$menarch add check -label "Ocultar/Mostrar Consola" -command "GUI::ocultarMostrarConsola $base" -variable GUI::${base}(consola)
		...
		$menu add cascade -label "Edición" -menu $menu.edicion
			...
			$menedit add command -label "Pegar" -command "GUI::pegar $base" -accelerator Ctrl-F2
			$menedit add separator
		...
		$menu add cascade -label "Tinker" -menu $menu.tinker
			...
			$mentink add cascade -label "Búsqueda Conformacional" -menu $mentink.busqconform 
				set menubc [menu $mentink.busqconform -tearoff 0]
				$menubc add command -label "Búsqueda Conformacional (Newton)" -command "GUI::modelizarTinker $base $baseConf 1" -command "GUI::modelizarTinker $base $baseConf 1" 
				$menubc add command -label "Búsqueda Conformacional (Optimizar)" -command "GUI::modelizarTinker $base $baseConf 2" -command "GUI::modelizarTinker $base $baseConf 2" 
			$mentink add command -label "Estado de Transición / Trayectoria de Reacción" -command "GUI::modelizarTinker $base $baseConf 5" 
			...
			$mentink add separator
			$mentink add command -label "Editar Log" -command "GUI::abrirFichLog $base"
			$mentink add command -label "Editar Key" -command "GUI::editarTinker $base"
		...
		$menu add cascade -label "Mopac" -menu $menu.mopac
			...
			$menmop add command -label "Mostrar Distribución de Cargas" -command "GUI::modelizarMopac $base $baseConf 1"
			$menmop add command -label "Visualizar Vibraciones" -command "GUI::modelizarMopac $base $baseConf 2"
			$menmop add separator
		...
		$menu add cascade -label "Ver" -menu $menu.ver
			...
			$menver add cascade -label "Estilo" -menu $menver.est
				...
				$menest add command -label "Estructura en Líneas" -command "GUI::manejadorMenuIzq $base lineas"
			...
			$menver add command -label "Puentes de Hidrógeno" -command "GUI::ventanaPuentesHidrogeno $base"
			#----------------
		...
		$menu add cascade -label "Herramientas" -menu $menu.herramientas
			...
			$menherr add cascade -label "Hidrógenos" -menu $menherr.hidrogenos
				set menhid [menu $menherr.hidrogenos -tearoff 0]
				$menhid add command -label "Añadir" -command "GUI::anadirHidrogenos $base"
				$menhid add command -label "Quitar" -command "GUI::quitarHidrogenos $base"
			$menherr add separator
			$menherr add cascade -label "Medición" -menu $menherr.medicion
				set menmed [menu $menherr.medicion -tearoff 0]
				$menmed add command -label "Distancia" -command "GUI::manejadorMedidas $base 0"
				$menmed add command -label "Ángulo" -command "GUI::manejadorMedidas $base 0"
				$menmed add command -label "Torsión" -command "GUI::manejadorMedidas $base 0"
			...
			$menherr add cascade -label "Etiquetas" -menu $menherr.etiquetas
				...
				$menetq add check -label "Cod. BrandyMol" -variable "GUI::${base}(etqCodB)" -command "GUI::manejadorEtiquetas $base CODB"
			...
			$menherr add separator
			$menherr add command -label "Superponer Moléculas" -command "GUI::modelizarTinker $base $baseConf 6"
			$menherr add separator
			$menherr add command -label "Mapa de Reacción" -command "GUI::ventanaMapaReaccion $base"
		
		$menu add cascade -label "Ayuda" -menu $menu.ayuda
		
		set menayu [menu $menu.ayuda -tearoff 0]
		$menayu add command -label "Ayuda" -command "GUI::manejadorAyuda $base Acerca_de"
		...
		set boxcaja1 [ButtonBox $frameb.bbox10 -spacing 0 -padx 1 -pady 1]
			$boxcaja1 add -image $Icon::especicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
				-padx 1 -pady 1 -helptext "Imagen Especular de la Molécula" -command "GUI::imagenEspecular $base"
			$boxcaja1 add -image $Icon::supericon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
				-padx 1 -pady 1 -helptext "Superponer Moléculas" -command "GUI::modelizarTinker $base $baseConf 6"
			...
			$boxcaja1 add -image $Icon::trashicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
				-padx 1 -pady 1 -helptext "Elimina las Moléculas" -command "GUI::eliminaMoleculas $base"
		...
		$boxcaja1 add -image $Icon::guardarcomoicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Guarda la Molécula" -command "GUI::manejadorCajaBot1 $base guardar"
		...
		$boxcaja1 add -image $Icon::helpicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Ayuda" -command "GUI::manejadorAyuda $base Acerca_de"
		...
		set gui(menuMedir) [menu .popupMenumedir -tearoff 0]
			...
			$gui(menuMedir) add command -label "Ángulo" -command "GUI::manejadorMedidas $base 0"
			$gui(menuMedir) add command -label "Torsión" -command "GUI::manejadorMedidas $base 0"
		...
		set gui(medirMenu) [$boxcaja1 add -image $Icon::rulericon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Activa el Modo de Medición\nDistancias / Ángulos / Torsiones" ]
		...
		$boxcaja1 add -image $Icon::renumicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Mapa de Reacción" -command "GUI::ventanaMapaReaccion $base"
		$boxcaja1 add -image $Icon::hidroicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Añadir Hidrógenos" -command "GUI::anadirHidrogenos $base"
		$boxcaja1 add -image $Icon::nohidroicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Quitar Hidrógenos" -command "GUI::quitarHidrogenos $base"
		#empaqueto
		pack $boxcaja1 -side left -anchor w
		...
		$boxcaja1 add -image $Icon::metanoicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Modelizar" -command "GUI::modelizarTinker $base $baseConf 0" 
			
		set gui(menuScan) [menu .popupMenuscan -tearoff 0]
			$gui(menuScan) add command -label "Búsqueda Conformacional (Newton)" -command "GUI::modelizarTinker $base $baseConf 1"
			$gui(menuScan) add command -label "Búsqueda Conformacional (Optimizar)" -command "GUI::modelizarTinker $base $baseConf 2"
			
		set gui(scanMenu) [$boxcaja1 add -image $Icon::scanicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Búsqueda Conformacional" ]
		...
		$boxcaja1 add -image $Icon::tsicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Estado de Transición y\nTrayectoria de Reacción" -command "GUI::modelizarTinker $base $baseConf 5"
		$boxcaja1 add -image $Icon::iricon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Calcula las Vibraciones de la Molécula" -command "GUI::modelizarTinker $base $baseConf 3"
		...
		set gui(mopacMenu) [$boxcaja1 add -image $Icon::mopacicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Cálculo con MOPAC" -command "GUI::modelizarMopac $base $baseConf 0"]
		...
		$boxcaja1 add -image $Icon::cargaicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Muestra la Distribución de Cargas" -command "GUI::modelizarMopac $base $baseConf 1"
		$boxcaja1 add -image $Icon::chimeicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
		...
		$boxcaja1 add -image $Icon::fittoviewicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Centra las Moléculas en la Pantalla" -command "GUI::manejadorMenuIzq $base centrar"
		...
		$boxcaja1 add -image $Icon::lineicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Muestra la Estructura en Líneas" -command "GUI::manejadorMenuIzq $base lineas"
		$boxcaja1 add -image $Icon::stickicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Muestra la Estructura en Cilindros" -command "GUI::manejadorMenuIzq $base cilindros"
		$boxcaja1 add -image $Icon::bysicon  -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Muestra la Estructura en Bolas y Cilindros" -command "GUI::manejadorMenuIzq $base cilYbolas"
		$boxcaja1 add -image $Icon::cpkicon  -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Muestra la Estructura en CPK" -command "GUI::manejadorMenuIzq $base cpk"
		...
		set gui(botonEjes) [$boxcaja1 add -image $Icon::ejesicon -activebackground #a0a0a0 -takefocus 0 -relief sunken -borderwidth 1 \
			-padx 1 -pady 1 -helptext "Mostrar/Quitar los Ejes de Coordenadas" -command "GUI::manejadorMenuIzq $base ejes"]
		...
		$mainframe addindicator -text "Brandy Mol 1.0"
		pack $framecent -side top -anchor n -fill both -expand yes
	...
	proc ventanaCteAcoplamiento { base } {
		...
		if { $ang == -1 || [cteAcoplamientoValida $base] == -1} {
			tk_messageBox -type ok -title "Atención" -icon warning -title "Ángulo Dihedro NO Seleccionado" -message "Seleccione 4 Átomos Correctos para Calcular la Cte. de Acoplamiento" 
		} else {
			set mainf ".cteacopl"
			...
			set label [Label $framecent.lab -activebackground #a0a0a0 -text "Ángulo Dihedro :   $ang \º" -font {Arial 10 {bold roman}} -relief flat -anchor w]
			set label2 [Label $framecent.lab2 -activebackground #a0a0a0 -text "Cte. de Acoplamiento :   J = [cteAcoplamiento $ang] Hz" -font {Arial 10 {bold roman}} -relief flat -anchor w]
			...
			set buttonAyuda [Button $framedown.ayuda -activebackground #a0a0a0 -helptext "Ayuda" -image $Icon::ayudicon -command "GUI::manejadorAyuda $base Acoplamiento"]
	...
	proc ventanaMapaReaccion { base } {
		...
		#creo la ventana
		toplevel $mainf
		wm title $mainf "Mapa de Reacción"
		#wm resizable $mainf 0 0
		#la ventana constara de 3 frames, superior, central e inferior
		...
		#fichero base
		set le1 [ LabelEntry $framecent1.le1 -label "Fichero 1 : " -textvariable GUI::${base}(ficheroMR1) -side left -labelfont {Arial 10 {bold roman}}]
		...
		#fichero trabajo
		set le2 [ LabelEntry $framecent2.le2 -label "Fichero 2 : " -textvariable GUI::${base}(ficheroMR2) -side left -labelfont {Arial 10 {bold roman}}]
		...
		#f1
		set l3a [Label $framecent3a.lea -activebackground #a0a0a0 -text "Átomo 1 : " -font {Arial 9 {bold roman}} -relief flat -anchor center]
		...
		#f2
		set l3b [Label $framecent3b.leb -activebackground #a0a0a0 -text "Átomo 2 :  " -font {Arial 9 {bold roman}} -relief flat -anchor center]
		...
		#f4
		set l3d [Label $framecent3d.led -activebackground #a0a0a0 -text "Asociaciones : " -font {Arial 10 {bold roman}} -relief flat -anchor center]
		...
		set buttonAyuda [Button $framedown.ayuda -activebackground #a0a0a0 -helptext "Ayuda" -image $Icon::ayudicon -command "GUI::manejadorAyuda $base MapaReacc"]
			...
			if {$f1of2 == 1} {
				...
				if {$gui(baseDataMR1) != ""} {
					...
					if {[string match $formula1 $formula2]} {
						...
						tk_messageBox -type ok -title "Atención" -icon warning -message "La Fórmula Molecular de \"$filename\" NO Coindice con la de \"$gui(ficheroMR1)\"" -parent $gui(mainfMR)
					}
				} else {
					tk_messageBox -type ok -title "Atención" -icon warning -message "Defina antes el Fichero 1" -parent $gui(mainfMR)
				}
			}
		}
	...
	proc manejadorEjecutarMR { base } {
		...
		if { $gui(baseDataMR1) != "" && $gui(baseDataMR2) != ""} {
			...
			if {[llength $gui(listaAsignadosMR1)] != $datos(numAtomos)} {
				tk_messageBox -type ok -title "Atención" -icon warning -message "Asignación NO Completada. Asigne los Átomos Restantes e Inténtelo de Nuevo"
			} else {
			...
			tk_messageBox -type ok -title "Atención" -icon warning -message "Defina los Ficheros y Realice la Asignación"
		}
	}; #finproc

	proc manejadorCancelarMR { base } {
		upvar #0 GUI::$base gui
		if {[tk_messageBox -type yesno -title "Atención" -icon warning -message "Está Seguro de que Desea Salir y Perder el Trabajo Realizado"]} {
			destroy $gui(mainfMR)
		}
	}
...
#----------------------------------------------------------------------------------------------		
	...
	proc manejadorLBRE { base } {
		...
		if {$gui(modoInteraccion) == "rotar"} {
			...
			$menu add command -label "Cortar" -command "GUI::cortar $base"
			$menu add command -label "Copiar" -command "GUI::copiar $base"
			$menu add command -label "Pegar" -command "GUI::pegar $base"
			$menu add separator
			$menu add check -label "Proyección Paralela/Cónica" -variable "GUI::${base}(proyeccion)" -command "GUI::manejadorProyeccion $base"
			...
			$menu add command -label "Configuración" -command "GUI::ventanaConfiguracion $base"
	...
	proc generaListaPropiedades { base } {
		...
		if {[llength $molec] == 1} {
			...
			lappend res [list "Fórmula Molecular" "[join [join $form]]"]
			lappend res [list "Número de Átomos" $datos(numAtomos)]
			lappend res [list "Peso Molecular" "[Data::pesoMolecular $form] g/mol"]
			lappend res [list "Cod. InchI" $datos(inchi)]
			...
			if {[llength $lista] == 1} {
				...
				lappend res [list "ID" "$mol [expr $id + 1]"]
				lappend res [list "Cod. BrandyMol" [Data::codBrandy $mol $id]]
				lappend res [list "Símbolo" $datos(simbolo,$id)]
				...
				lappend res [list "ID" "($mol1 [expr $id1 + 1]) , ($mol2 [expr $id2 + 1])"]
				lappend res [list "Cod. BrandyMol" "[Data::codBrandy $mol1 $id1] , [Data::codBrandy $mol2 $id2]"]
				lappend res [list "Símbolo" "$datos1(simbolo,$id1), $datos2(simbolo,$id2)"]
				#lappend res [list "Coordenadas" "$datos(coordX,$id) $datos(coordY,$id) $datos(coordZ,$id)"]
				#lappend res [list "Carga" "$datos(carga,$id)"]
				...
				lappend res [list "ID" "($mol1 [expr $id1 + 1]) , ($mol2 [expr $id2 + 1]) , ($mol3 [expr $id3 + 1])"]
				lappend res [list "Cod. BrandyMol" "[Data::codBrandy $mol1 $id1] , [Data::codBrandy $mol2 $id2] , [Data::codBrandy $mol3 $id3]"]
				lappend res [list "Símbolo" "$datos1(simbolo,$id1), $datos2(simbolo,$id2), $datos3(simbolo,$id3)"]
				#lappend res [list "Coordenadas" "$datos(coordX,$id) $datos(coordY,$id) $datos(coordZ,$id)"]
				#lappend res [list "Carga" "$datos(carga,$id)"]
				lappend res [list "Ángulo" "[VisorVTK::insertarAngulo $gui(visor) [list $datos1(coordX,$id1) $datos1(coordY,$id1) $datos1(coordZ,$id1)] \
				...
				lappend res [list "ID" "($mol1 [expr $id1 + 1]) , ($mol2 [expr $id2 + 1]) , ($mol3 [expr $id3 + 1]), ($mol4 [expr $id4 + 1])"]
				lappend res [list "Cod. BrandyMol" "[Data::codBrandy $mol1 $id1] , [Data::codBrandy $mol2 $id2] , [Data::codBrandy $mol3 $id3], [Data::codBrandy $mol4 $id4]"]
				lappend res [list "Símbolo" "$datos1(simbolo,$id1), $datos2(simbolo,$id2), $datos3(simbolo,$id3), $datos4(simbolo,$id4)"]
				#lappend res [list "Coordenadas" "$datos(coordX,$id) $datos(coordY,$id) $datos(coordZ,$id)"]
				#lappend res [list "Carga" "$datos(carga,$id)"]
				lappend res [list "Ángulo Dihedro" "[VisorVTK::insertarTorsion $gui(visor) [list $datos1(coordX,$id1) $datos1(coordY,$id1) $datos1(coordZ,$id1)] \
																				  [list $datos2(coordX,$id2) $datos2(coordY,$id2) $datos2(coordZ,$id2)] \
			...
			if {[llength $lista] == 1} {
				...
				lappend res [list "Cod. BrandyMol" "[Data::codBrandy $mol1 $id1] , [Data::codBrandy $mol1 $id2]"]
				lappend res [list "Símbolo" "$datos1(simbolo,$id1), $datos1(simbolo,$id2)"]
				#lappend res [list "Coordenadas" "$datos(coordX,$id) $datos(coordY,$id) $datos(coordZ,$id)"]
				#lappend res [list "Carga" "$datos(carga,$id)"]
				lappend res [list "Distancia" "[VisorVTK::insertarDistancia $gui(visor) [list $datos1(coordX,$id1) $datos1(coordY,$id1) $datos1(coordZ,$id1)] \
	...
	proc ventanaPropiedades { base listaP } {
		...
		#frame inferior
		set framedown [frame $mainf.framedown -borderwidth 3 -bd 2 -relief raised]
		set butonayu [Button $framedown.ayuda -activebackground #a0a0a0 -helptext "Ayuda" -command "GUI::manejadorAyuda $base Visualizador" -image $Icon::ayudicon -text "Ayuda"]
	...
	proc menuEtiquetas { base boton } {
		...
		$menu add check -label "Cod. BrandyMol" -variable "GUI::${base}(etqCodB)" -command "GUI::manejadorEtiquetas $base CODB"
...
#----------------------------------------------------------------------------------------------		
	...
	proc ventanaConfiguracion { base } {
		...
		set ldata [Label $d1.l1 -activebackground #a0a0a0 -text "Directorio de Datos : " -font {Arial 10 {bold roman}} -relief flat -anchor center]
		...
		set lbin [Label $d2.l1 -activebackground #a0a0a0 -text "Directorio de Ejecutables : " -font {Arial 10 {bold roman}} -relief flat -anchor center]
		...
		set label4 [Label $fetq.l4 -activebackground #a0a0a0 -text "Color de las Etiquetas" -font {Arial 10 {bold roman}} -relief flat -anchor center]
		...
		set label4 [Label $fbg.l4 -activebackground #a0a0a0 -text "Color del Fondo    " -font {Arial 10 {bold roman}} -relief flat -anchor center]
	...
	proc manejadorCambiarDirData { base etq } {
		...
		set filename [tk_chooseDirectory -parent $gui(mainConfig) -initialdir $config(dirDataOrig) -title "Escoja el Directorio de Datos"]
		if {$filename != ""} {
	...
	proc guardarPropiedades { base } {
		...
		Conf::crearFichConf $gui(conf) "$config(dirBin)conf.ini" "$config(dirBin)colors.ini" $dirData $gui(colorFondo) $gui(colorEtq) $gui(elementos) $gui(colores) $gui(escalaCPK) $config(estadoConsola)
		destroy	$gui(mainConfig)
		tk_messageBox -type ok -title "Información" -message "Los Cambios se Efectuarán la Próxima Vez que Inicie BrandyMol"
...
#----------------------------------------------------------------------------------------------		
	...
	proc cargarFicheroMol { base filename } {
		...
		if { $erro == ""} {
			...
			tk_messageBox -type ok -icon warning -title "Atención" -message "Error al Leer el Fichero $filename, Compruebe que el Formato es el Correcto" 
			return "ERROR"
		}
	}; #finproc
	...
	proc guardarComo { base file } {
		...
		if {[llength $listaM] == 1} {
			...
			if {$file == 0} {
				set op [tk_messageBox -title "Información" -message "¿Desea Agrupar Todas las Moléculas en el Mismo Fichero?" -type yesnocancel -icon question]
			} else { set op "yes"}
			if {$op == "yes"} {
				if {$file == 0} {
					set filename [tk_getSaveFile -filetypes $types -defaultextension mol -initialdir $config(dirData) -title "Guardar Fichero Mol"]
	...
	proc editarTinker { base } {
		...
		if {[file exists $fkey]} {
			...
			set butCerrar [Button $framedown.ok -activebackground #a0a0a0 -command "destroy $mainf ; destroy $menuppal" -helptext "Cerrar la Ventana" -image $Icon::cancelicon]		
			...
			$menuppal add cascade -label "Insertar" -menu ${mainf}_menuppal.tink	
				...
				$menutink add command  -command "GUI::modificaPRMFichKey $base $texto" -label "Parámetros"
				$menutink add separator 
				$menutink add command  -command "$texto insert end {PISYSTEM   Escriba la lista de átomos PI \n}" -label "Sistema Pi"
				$menutink add separator 
				$menutink add command  -command "$texto insert end {RESTRAIN-DISTANCE  Escriba la lista de átomos \n}" -label "Fijar Distancia"
				$menutink add command  -command "$texto insert end {RESTRAIN-ANGLE  Escriba la lista de átomos \n}" -label "Fijar Ángulo"
				$menutink add command  -command "$texto insert end {RESTRAIN-TORSION  Escriba la lista de átomos \n}" -label "Fijar Torsión"
				$menutink add separator 
				$menutink add command  -command "$texto insert end {DIELECTRIC Escriba la constante dieléctrica (0.1 a 79)\n}" -label "Constante Dieléctrica"
				$menutink add command  -command "$texto insert end {MAXITER Escriba el máximo número de iteraciones \n}" -label "Iteraciones Máximas"
				$menutink add separator 
				$menutink add command  -command "$texto insert end {SADDLEPOINT \n}" -label "Estado de Transición"
				$menutink add command  -command "$texto insert end {REDUCE Escriba un valor entre 0 y 1 (aprox. 0.3) \n}" -label "Reduce"
				$menutink add command  -command "$texto insert end {ENFORCE-CHIRALITY \n}" -label "Forzar Quiralidad"
			...
			tk_messageBox -type ok -title "Error Critico!" -icon error -message "No Existe el Fichero Tinker.key!!!\
...
#----------------------------------------------------------------------------------------------			
	...
	proc ventanaPuentesHidrogeno { base } {
		...
		toplevel $mainf
		wm title $mainf "Puentes de Hidrógeno"
		...
		set label [Label $framecent.l1 -activebackground #a0a0a0 -text "Puentes Hidrógeno Disponibles" -font {Arial 11 {bold roman}} -relief flat -anchor center]
		
		set oxig [checkbutton $fa.oxig -offvalue N -onvalue Y -text "Oxígeno" -font {Arial 9 {bold roman}} -variable "GUI::${base}(phO)"]
		set nitro [checkbutton $fa.nitor -offvalue N -onvalue Y -text "Nitrógeno" -font {Arial 9 {bold roman}} -variable "GUI::${base}(phN)"]
		set azuf [checkbutton $fa.azuf -offvalue N -onvalue Y -text "Azufre" -font {Arial 9 {bold roman}} -variable "GUI::${base}(phS)"]
		set flu [checkbutton $fa.flu -offvalue N -onvalue Y -text "Flúor" -font {Arial 9 {bold roman}} -variable "GUI::${base}(phF)"]
		...
		set label [Label $fa.l2 -activebackground #a0a0a0 -text "Distancia de Formación" -font {Arial 11 {bold roman}} -relief flat -anchor center]
...
#----------------------------------------------------------------------------------------------			
	...
	proc modelizarTinker { base baseConfig tipo } {
		...
		if {$gui(semaforoProceso) == 0} {
			#set gui(semaforoProceso) 1
			activaDesactivaSemaforo $base 1 "Tinker en Curso..."
			if {[string match [$gui(labelFichActual) cget -text] "Fichero no Definido"] && $tipo != 4 && $tipo != 5 && $tipo != 6 } {
				...
				if {[llength $gui(fichCargados)] > 0} {
					...
					} else {activaDesactivaSemaforo $base 0 "Listo"}
				} else {
			...
			tk_messageBox -title "Información" -message "Existe un Cálculo en Proceso, Deténgalo o Espere a que Finalize."
		}
	}; #finproc
	...
	proc finProcesoTink { base } {
		...
		activaDesactivaSemaforo $base 1 "Proceso Tinker en Curso..."
		...
		activaDesactivaSemaforo $base 0 "Listo"
	}; #finproc
	...
	proc modelizarMopac { base baseConfig tipo } {
		...
		if {$gui(semaforoProceso) == 0} {
			activaDesactivaSemaforo $base 1 "Mopac en Curso..."
			...
			if {[string match [$gui(labelFichActual) cget -text] "Fichero no Definido"] && $tipo != 1} {
				...
				if {[llength $gui(fichCargados)] > 0} {
					...
					#aunk no haya nada cargado en el visor, dejare ver la ventana de Tinker
					Mopac::newMopac m d $gui(conf) $base "Fichero no Definido" $tipo
				}
	...
	proc finProcesoMopac { base } {
		...
		activaDesactivaSemaforo $base 1 "Proceso Mopac en Curso..."
...
#----------------------------------------------------------------------------------------------		
	...
	proc acercaDe { base } {
		...
		set texto3 "    BrandyMol ha sido desarrollado como PFC \n \
						de la titulación Ingeniería Técnica en Informática \n \
						de Sistemas para el Dpto. de Química Orgánica \n \
		...
		set texto4 "Agradecimientos Especiales: \n \
					...
					\   Mopac, F2C, Inno Setup, Microsoft y Notepad++"
