...
#		base(renderW)			--> vtkRenderWindow
#		base(renderWInt)		--> vtkRenderWindowInteractor
#		base(intStyle)			--> vtkInteractorStyle
#		base(picker)			--> vtkPicker
#		base(propPicker)		--> vtkPropPicker
#		base(lookupTable) 		--> vtkLookupTable
#
#
#		Contenedores de Informacion
#
#
#		base(moleculas) 			--> lista con los nombres de las moleculas que contiene el visor
#		base(ficheroSeleccionado) 	--> nombre del fichero de la molecula seleccionada, solo actualizada mediante doble click
#		
#		base(ladosTubeFilter) 		-->  numero de lados que tendran los tubos que representan los enlaces
#		base(escalaCPK) 			--> escala de las esferas que representan los atomos en CPK
#		
#		#texto 2D del visor y medidas
#		base(actoresTexto) 			--> lista de los actores texto que representan contenido numerico
#		base(semillaActoresTexto) 	--> semilla para el numero de actores texto
#		base(actoresSeleccMedidas)	--> lista de los ultimos atomos seleccionados, de longitud maxima : 4
#		base(listaMedidas) 			--> lista de 2,3,4-tuplas, de actoresAtomos que representaran las medidas activas.
#											2 : distancia, 3 : angulo, 4: Torsion#		
#		#etiquetas
#		base(listaEtiquetas) 		--> lista de pares "texto actorAtomo" que indican que texto esta asociado a que atomo
#		base(actoresEtiquetas) 		--> lista de actores (vtkFollower, siempre miran a camara) de las etiquetas
#		base(etqId) 				--> indica si deben mostrarse los identificadores en las etiquetas
#		base(etqCodB)  				--> indica si debe mostrarse el codigo BrandyMol en las etiquetas
#		base(etqCarga)  			--> indica si debe mostrarse las cargas en las etiquetas
#		base(etqCodTink)  			--> indica si debe mostrarse el codigo Tinker identificadores en las etiquetas
#		base(etqQuira)  			--> indica si debe mostrarse la Quiralidad en las etiquetas
#		
#		#puentes de hidrogeno
#		base(listaPuentesH) 	--> lista de pares de actoresAtomos que representaran los puentes de hidrogeno.
#		base(elemsPuentesH) 	--> lista de elementos que formaran puentes los puentes de hidrogeno
#		base(distPuentesH) 		--> distancia minima de formacion para los PH
#		
#		#opciones de interaccion
#		base(opcionAct) 			--> opcion que esta cursandose, seleccionar, rotar, desplazar, centrar, o zoom
#		base(SHIFT)  				--> estado de la tecla SHIFT, 0: no pulsado, 1: pulsado
#		base(CTRL)  				--> estado de la tecla CTRL, 0: no pulsado, 1: pulsado
#		base(ratonIzqPuls) 			--> estado del boton derecho del raton, 0: no pulsado, 1: pulsado
#		base(ratonDchoPuls) 		--> estado delboton derecho del raton, 0: no pulsado, 1: pulsado
#		base(DobleClick) 			--> estado del doble click 0: no producido, 1: producido
#		base(ratonMov) 				--> estado del movimiento del raton, 0: no se ha movido desde que se se pulso
#									el boton izq del raton, 1: en caso contrario
#		base(agregaSeleccion) 		--> 0: deseleccionar todo seleccionar segun el lazo de seleccion
#									    1: agregar el lazo de seleccion a lo ya seleccionado
#		base(actorOpcMover) 		--> actor seleccionado para una operacion de movimiento
#		
#		#ejes de rotacion usados en la interaccion
#		base(ejesRot) 				--> contiene una lista de cuatro elementos "centro ejeX ejeY ejeZ" de unos ejes  
#						  		 	centrados en la BB de todo lo seleccionado, con respecto a la camara
#				
#		#orbitales
#		base(orbitales) 			--> lista de nombres de orbitales
#
#		#luces
#		base(luces) [list]			--> lista de nombres de luces
#
#		#superficies
#		base(superficies) [list] 	--> lista de nombres de superfices
#		base(semillaSuperficies) 	--> semilla para la generacion de superficies
#		
#		#lazo de seleccion
#		base(rSelect) 				--> indica si el rectangulo de seleccion debe ser mostrado
#		base(rSelectI) 				--> indica la posicion inicial en pixeles de un cuadro de seleccion
#		base(rSelectF) 				--> indica la posicion final en pixeles de un cuadro de seleccion
#
#		base(last) 					--> indica la ultima posicion del cursor del raton
#
#			
#		Objetos VTK
#
#		base(ejesCoord) 	--> vtkAxesActor, actor que define unos ejes de coordenadas
#		base(ejesMarker) 	--> vtkOrientationMarkerWidget, filtra los eventos sobre los ejes permitiendo solo las rotaciones
#
#		base(atomoCB) 		--> vtkSphereSource, source de la esfera que representara los atomos
#		base(atomoMapperCB) --> vtkPolyDataMapper, mapper de la esfera que reperesentara los atomos
#		base(enlacePoints) 	--> vtkPoints, puntos para formar una linea vertical que forme la base para los enlaces
#		base(enlaceCells) 	--> vtkCellArray  linea de 4 puntos que representa la base de un enlace
#			
#		base(rSelectPoints)		--> vtkPoints, para representar los puntos de un lazo de seleccion
#		base(rSelectCells)	 	--> vtkCellArray, para representar las lineas de un lazo de seleccion
#		base(rSelectPolyData) 	--> vtkPolyData, polydata del lazo de seleccion
#			
#		base(rSelectPolyDataMapper) --> vtkPolyDataMapper2D,  mapper 2D para el lazo de seleccion
#		base(rSelectActor) 			--> vtkActor2D, del lazo de seleccion
#			
#		
#		base(medidasPoints) 		--> vtkPoints  para representar los puntos que forman las medidas
#		base(medidasCells)			--> vtkCellArray 	 para representar las lineas que forman las medidas
#		base(medidasPolyData)		--> vtkPolyData de las medidas
#		base(medidasPolyDataMapper) --> vtkPolyDataMapper, mapper de las medidas
#		base(medidasActor) 			--> vtkActor, actor de las medidas
#			
#		base(puentesHPoints) 		--> vtkPoints  para representar los puntos que forman las medidas
#		base(puentesHCells) 		--> vtkCellArray  para representar las lineas que forman las medidas
#		base(puentesHPolyData) 		--> vtkPolyData de las medidas
#		base(puentesHPolyDataMapper)--> vtkPolyDataMapper, mapper de las medidas
#		base(puentesHActor) 		--> vtkActor, actor de las medidas
#
#
#
...
#		base(actoresAtomos,mol)		--> lista de nombres de actores VTK, que representan los atomos de la molecula mol
#		base(numEnlaces,mol)		--> numero de atomos de la molecula mol
#		base(actoresEnlaces,mol)	--> lista de nombres de actores VTK, que representan los atomos de la molecula mol
#
#		base(estadoRepres,mol,x)	--> estado de representacion en que se encuentra el atomo x, L, C, CB o CPK de la molecula mol
#		base(estadoRepresEn,mol,i,j)--> estado de representacion en que se encuentra el enlace i,j : L, C, CB de CPK de la molecula mol
...
namespace eval VisorVTK {
	...
	#crea una instancia definiendo un array VisorVTK::$base, y creando los objetos basicos VTK
	proc newVisorVTK { base } {
	...
	#elimina tanto los datos contenidos en el visor como la estructura.
	proc delVisorVTK { base } {
	...
	#Carga la vtkLookupTable del visor base, con la información referente a los colores contenida en la instancia de Conf baseConf
	proc inicializarLookupTable { base baseConf } {
		upvar #0 VisorVTK::$base visor
	...
	proc inicializarVisor { base } {
		...
		#creo una esfera que representara los atomos
		...
		#creo una cell q representa una polyline de 3 tramos, centrada en 0 0 0
	...
	#actualiza el interactor al de la ventana activa y asocia los eventos tipicos para un visor molecular completo
	...
	#actualiza el interactor al de la ventana activa y asocia los eventos tipicos para un visor molecular minimo, como el usado para los orbitales
	...
	#devuelve en nombre de la vtkRenderWindow
	proc devuelveRenderWindow { base } {
		upvar #0 VisorVTK::$base visor
		return $visor(renderW)
	}; #finproc
	
	#carga la molecula baseData en lel visor
	proc cargaVisorMolData { base baseData baseConf } {
	...
	#carga el orbital definido en filename, bajo el nombre nombreOrbital
	proc cargarOrbitalFich { base filename nombreOrbital } {
		upvar #0 VisorVTK::$base visor
	...
	proc manejadorKPE { base key } {
		...
		switch $key {
			...
			Control_L - Control_R {
				...
				if {$visor(opcionAct) == "mover"} {
					if {[devolverEstadoEjes $base] == 1} {ejesInteractivos $base 1}
				} 				
	...
	proc manejadorKRE { base key } {
		...
		switch $key {
			...
			Control_L - Control_R {
				...
				if {$visor(opcionAct) == "mover"} {
					if {[devolverEstadoEjes $base] == 1} {ejesInteractivos $base 0}
				} 				
				set visor(CTRL) 0
			}
			default {
