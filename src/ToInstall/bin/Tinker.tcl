...
#		base(baseData) 			--> nombre del Data asociados
#
...
#
#		base(progEjec)			--> programa seleccionado para ejecutar
#		base(nombreCalculo) 	--> Nombre del calculo a Ejecutar
#		base(cancelaProceso)	--> 1 si el proceso ha sido cancelado por el usuario
#		
#		para la modelizacion
#		base(prog)				--> Opcion selecionada mediante el ComboBox : Newton, Optimizar, Minimizar o Analisis
...
#		base(mismaVentana)				--> Y/N todas las vibraciones en la misma ventana o en ventanas distintas
#
...
#		base(comboFicherosBaseSuper)		--> nombred el combo de superpose
#		base(ficheroBaseSuper)				--> fichero base del superpose
#		base(entryFicherosTrabSuper)		--> direcotrio de trabajado del superpose
#		base(textoWSuper)					--> nombre del Widget con los enlaces seleccionados
#		base(listaFicherosTrabSuper) 		--> lista de todos los los ficheros seleccionados para el superpose
#		base(listaComboFicherosTrabSuper) 	--> Lista con los nombres (sin la ruta) de los ficheros seleccionados para el superpose
#
...
namespace eval Tinker {
	...
	proc newTinker { base baseData baseConf fichMOL baseGui tipo } {
		...
		if {[info exists Tinker::$base] == 0} {
			...
			set ${base}(fichero3Mol) "Estado de Transición Previo"
			...
			set ${base}(entryFicherosTrabSuper) "Elija Ficheros para la Superposición"
			...
			switch $tipo {
				1 { set ${base}(tipoBC) 1; ventanaBusquedaConformacional $base "Búsqueda Conformacional (Newton)" }
				2 { set ${base}(tipoBC) 2; ventanaBusquedaConformacional $base "Búsqueda Conformacional (Optimizar)" }
	...
	proc ventanaModelizarTinker { base } {	
		...
		set ncalc [ LabelEntry $frameup2.calculo -label "Nombre del Cálculo TINKER : " -textvariable "Tinker::${base}(nombreCalculo)" -side left -width 40 -labelfont {Arial 9 {bold roman}}]			
		...
		set batch [checkbutton $frameup.batch -offvalue N -onvalue Y -text "Varios Ficheros" -variable Tinker::${base}(batch)]
		...
		set metodol [ LabelFrame $metodo.metodol -text "Método" -side left ]
		...
		set ima1 [Label $metodo.ima -activebackground #a0a0a0 -helptext "Modelizar" -image $Icon::metanoicon -anchor center -relief flat -height 16 -width 16]
		...
		set interindi [radiobutton $frameupb.interindi -text "Interac. Individuales" -value D -variable Tinker::${base}(analisis) -state $tink(estadoAnalisis)]
		set ff [radiobutton $frameupb.ff -text "Campo de Fuerza" -value P -variable Tinker::${base}(analisis) -state $tink(estadoAnalisis)]
		set ima2 [Label $frameupb.ima -activebackground #a0a0a0 -helptext "Analizar" -image $Icon::analicon -relief flat -anchor center -height 16 -width 16 -state $tink(estadoAnalisis)]
		...
		set ima3 [Label $framemid.ima -activebackground #a0a0a0 -helptext "Área y Volumen" -image $Icon::volumeicon -relief flat -anchor center -height 16 -width 16  -state $tink(estadoAnalisis)]
		...
		$menuinsert add command  -command "Tinker::modificaPRMFichKey $base $textokey $texto" -label "Parámetros"
		...
		set butonayu [Button .tinkerq.butonframe.ayuda -activebackground #a0a0a0 -helptext "Ayuda" -image $Icon::ayudicon -text "Ayuda" -command "Tinker::manejadorAyuda $base Modelizar"]
		...
		#establezco un manejador para cuando termine el proceso de tinker, este avisara a la gui de q ha terminado
	...
	proc manejadorEjecutarModelizar { base } {
		...
		if {$tink(batch) == "N"} {
			...
			if {[file exists "[file dirname $tink(fichMol)]/$tink(nombreCalculo).mol"] && 
			   ![tk_messageBox -type yesno -message "El Cálculo \"$tink(nombreCalculo)\" ya Existe.\nSi Continúa se Borrará, ¿Está Seguro de Que Desea Continuar?" -parent $tink(mainframe)]} {			   
				set calc 0
			}
	...
	proc ventanaBusquedaConformacional { base title } {
		...
		set auto [radiobutton $fcizq.auto -text "Automática" -value 0 -command "Tinker::estadoBotonesBC $base" -variable Tinker::${base}(seleccion) ]		
		...
		set direcl [ LabelEntry $fccen.direc -label "Direc.: " -textvariable Tinker::${base}(direc) -side left -justify right]		
	...
	proc ventanaVibraciones { base } {
		...
		set butonayu [Button $frameupa.ayuda -activebackground #a0a0a0 -helptext "Ayuda" -image $Icon::ayudicon -text "Ayuda" -command "Tinker::manejadorAyuda $base Vibraciones_Tinker"]		
		set butonejec [Button $frameupa.ok -activebackground #a0a0a0 -command "Tinker::manejadorBotVibracion $base" -helptext "Mostrar la Vibración" -image $Icon::okicon]		
	...
	proc ventanaETTR { base } {
		...
		wm title .ts "Estado de Transición y Trayectoria de Reacción"
		wm resizable .ts 0 0
		...
		set ncalc [ LabelEntry $frameup2.calculo -label "Nombre del Cálculo TINKER : " -textvariable "Tinker::${base}(nombreCalculo)" -side left -width 40 -labelfont {Arial 9 {bold roman}} -state disabled]			
		...
		set radio1 [radiobutton $framecentizq.r1 -text "Estado de Transición" -value ET -variable Tinker::${base}(seleccionETTR)]
		set radio2 [radiobutton $framecentizq.r2 -text "Trayectoria de Reacción" -value TR -variable Tinker::${base}(seleccionETTR)]
		set check1 [checkbutton $framecentizq.c1 -offvalue N -onvalue Y -text "Partir de un Estado de Transición Previo" -variable Tinker::${base}(transicionPrev)]
		set lab2 [Label $framecentizq.lab2 -activebackground #a0a0a0 -text "Selección del Cálculo : " -font {Arial 9 {bold roman}} -relief flat -anchor w]
		pack $lab2 -ipady 10
		...
		set butonayu [Button $framedown.ayuda -activebackground #a0a0a0 -helptext "Ayuda" -image $Icon::ayudicon -text "Ayuda" -command "Tinker::manejadorAyuda $base Estado_de_Transicion"]
	...
	proc ventanaSuperponer { base } {
		...
		set ncalc [ LabelEntry $frameup2.calculo -label "Nombre del Cálculo TINKER : " -textvariable "Tinker::${base}(nombreCalculo)" -side left -width 20 -labelfont {Arial 9 {bold roman}} -state disabled]			
		...
		set b1 [Button $fb1.b1 -activebackground #a0a0a0 -command "Tinker::manejadorBotonFicheroSuperponer $base" -helptext "Escoja los Ficheros" -image $Icon::diricon -text "Ayuda"]		
		...
		$tink(textoWSuper) insert end "      Introduzca los Enlaces Aquí"
		...
		set butonayu [Button $framedown.ayuda -activebackground #a0a0a0 -helptext "Ayuda" -image $Icon::ayudicon -text "Ayuda" -command "Tinker::manejadorAyuda $base Superponer"]		
	...
	proc manejadorEjecutarSuperponer { base } {
		...
		if {![file exists $tink(fichero1Mol)]} {
			...
			if {$calc == 1} { 
				...
				destroy $tink(mainframe)
				tk_messageBox -type ok -message "Cálculo Finalizado" -title "Información"
...
#----------------------------------------------------------------------------------------------		
	...
	proc ejecutaTinkerBatch { base } {
		...
		set filenames [tk_getOpenFile -filetypes $types  -multiple 1 -initialdir $config(dirData) -title "Abrir Varios Ficheros"]		
	...
	proc tratarResultadoTinker { base log alpha modif } {
		...
		if {$gui(cancelarProceso) == 1} {
			...
			tk_messageBox -type ok -message "Cálculo Cancelado"
			#set res "ERROR"
		} else {
			...
			switch $tink(prog) {
				...
				Optimizar {
					...
					if {[file exists "${alpha}_2"]} {								
						...
						if {$tink(batch) == "N"} {tk_messageBox -type ok -message "Cálculo Finalizado"  -title "Información"}
						set res [list "[file dirname $tink(fichMol)]/[file tail $nuevoMOL]"]
					} else {
						tk_messageBox -type ok -icon warning -title "Error en el Cálculo" -message "Ha Ocurrido un Error Durante el Cálculo. Mire el Fichero Log"						
						set res "ERROR"
					}
				}
				Analizar {
					...
					if {![eof $l]} {
						...
						gets $l linea
						append analisis $linea
						tk_messageBox -type ok -icon warning -title "Área y Volumen" -message $analisis
					} else {
						if {$tink(batch) == "N"} {tk_messageBox -type ok -message "Cálculo Finalizado"  -title "Información"}
					}
					close $l
				...
				#creo una carpeta TS en el direcotiro del 1º fichero proporcionado
					...
					if {$tink(transicionPrev) == "N" && [file exists tstate.xyz]} {
						...
						tk_messageBox -type ok -message "Cálculo Finalizado"
						...
						tk_messageBox -type ok -icon warning -message "No Se Ha Encontrado el Estado de Transición" -title "Información"
						set res "ERROR"
					}
					#borro el cualquier rastro de tstate
					set lis [glob -nocomplain "tstate.*"]
			...
			if {$tink(batch) == "N"} {
				...
				if {$tink(prog) == "Scan" } {
					...
					Log::anadirBotonLog log_$gui(semillaLogs) "Ordena el Fichero Log de una\nBúsqueda Conformacional" "$Icon::ordenicon" $cmd					
				}
	...
	proc tratarResultadoTinkerCancelado { base log alpha modif } {
		...
		#
			...
			if {$recuperado != ""} { tk_messageBox -message "Los Ficheros Scan Se Han Recuperado Satisfactoriamente en \"$scanDir\"" }			
