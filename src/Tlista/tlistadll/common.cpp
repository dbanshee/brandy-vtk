   ...
   if (dwNumTasks == 0)
    ...
    if (hKernel)
    {
        pCreateToolhelp32Snapshot =
          (CREATESNAPSHOT)GetProcAddress((struct HINSTANCE__ *)hKernel, "CreateToolhelp32Snapshot");

        pProcess32First = (PROCESSWALK)GetProcAddress((struct HINSTANCE__ *)hKernel,
                                                      "Process32First");
        pProcess32Next  = (PROCESSWALK)GetProcAddress((struct HINSTANCE__ *)hKernel,
                                                      "Process32Next");
    }
    ...
    )
{
    ...
    // kill the process
    //
    PostMessage((struct HWND__ *)tlist->hwnd, WM_CLOSE, 0, 0 );
    ...
    )
{
    ...
    //EnumWindows( (struct HWND__ *)EnumWindowsProc, (LPARAM) te );
}
...

{
    ...
    for (i=0; i<numTasks; i++) {
	    ...
	    // we found the task so lets try to get the
            // window text
            //
            if (GetWindowText( (struct HWND__ *)tlist[i].hwnd, buf, sizeof(buf) )) {
                //
