...
namespace eval GUI {
	...
	proc newGUI { base baseVisor baseConf } {
		...
		set gui(menuScan) [menu .popupMenuscan -tearoff 0]
			$gui(menuScan) add command -label "Búsqueda Conformacional TNCG" -command "GUI::modelizarTinker $base $baseConf 1"
			$gui(menuScan) add command -label "Búsqueda Conformacional OCVM" -command "GUI::modelizarTinker $base $baseConf 2"
		...
		#$boxcaja1 add -image $Icon::tsicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
		#	-padx 1 -pady 1 -helptext "Estado de Transición (SADDLE)" -command "GUI::wio"
		#$boxcaja1 add -image $Icon::iricon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
		#	-padx 1 -pady 1 -helptext "Calcula las Vibraciones de la Molécula" -command {tk_messageBox -type ok -message "Abrir" }
		$boxcaja1 add -image $Icon::chimeicon -activebackground #a0a0a0 -takefocus 0 -relief link -borderwidth 1 \
	...
	proc editarTinker { base } {
		...
		if {[file exists $fkey]} {
			...
			$menuppal add cascade -label "Insertar" -menu ${mainf}_menuppal.tink	
				...
				$menutink add command  -command "GUI::modificaPRMFichKey $base $texto" -label "Parámetros" -underline 0
...
#----------------------------------------------------------------------------------------------		
	...
	proc acercaDe { base } {
		...
		set texto3 "    Brandy Mol ha sido desarrollado como PFC de la \n \
						...
						para el Dpto de Química Orgánica de la Universidad\n \
						de Málaga"
